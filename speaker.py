...
class ResponsiveSpeakerGenerator(nn.Module):
...
class InputFusionModule(nn.Module):
    ...
    def forward(self, audio_seq):
        ...
        fused_seq = audio_emb
        init_state = audio_emb.mean(dim=1)
        return fused_seq, init_state

class SpeakerGenerationModel(nn.Module):
    def __init__(self, audio_dim, d_model=256, num_heads=4, hidden_dim=256, num_steps=60, out_dim_upper=50, out_dim_lower=50, dropout=0.1):
        super().__init__()
        self.input_fusion = InputFusionModule(audio_dim, d_model)
        self.responsive_gen = ResponsiveSpeakerGenerator(d_model=d_model, num_heads=num_heads, hidden_dim=hidden_dim, num_steps=num_steps, out_dim_upper=out_dim_upper, out_dim_lower=out_dim_lower, dropout=dropout)
    def forward(self, audio_seq, bias_mask=None):
        fused_seq, init_state = self.input_fusion(audio_seq)
...
if __name__=='__main__':
    ...
    model = SpeakerGenerationModel(audio_dim=audio_dim, d_model=d_model, num_heads=4, hidden_dim=256, num_steps=T, out_dim_upper=50, out_dim_lower=50, dropout=0.1)
    beta_pred, rho_pred = model(audio_seq, bias_mask=bias_mask)
